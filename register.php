...
if(!empty($_SESSION['LoggedIn']) && !empty($_SESSION['uname']))
    ...
    else
    {
     ...
     $fetch = $dbinfo->prepare("SELECT * from login WHERE uname=?");
...
else 
{
	...
	if(!empty($_POST['username']) && !empty($_POST['password']) && !empty($_POST['email']))
    				...
    				$email =$_POST['email'];
    				$checkusername = $dbinfo->prepare("SELECT * FROM login WHERE uname = ?");
	 					$checkusername->execute(array($username));
	 					$checkemail = $dbinfo->prepare("SELECT * FROM login WHERE uemail = ?");
        			...
        			echo "<p>Sorry, that username is taken. Please go back and try again.</p>";
        				...
        				echo "<p>Sorry, that email address is already in use. Please go back and try again.</p>";
     					...
     					{
     					$registerquery = $dbinfo->prepare("INSERT INTO login (uname, upass, uemail) VALUES(?, ?, ?)");
